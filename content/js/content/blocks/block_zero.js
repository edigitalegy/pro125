...
function createCustomButtons(option) {
  const buttonData = {
      ...
      6: { id: "0selflearning", icon: "bi-person-workspace", text: "أداة التفكير التكنولوجي",time:"" },
      7: { id: "0map", icon: "bi-diagram-3", text: "أداة الخريطة الذهنية", time: "" }
    };
...
{
  if(button.id === "0btnpretest"){ loadForm('https://forms.gle/rTGqn7Lw3RsesGiJ7'); }
  if(button.id === "0btnposttest"){ loadForm('https://forms.gle/h5ZGfgHwnk1X6CxHA');}
  if(button.id === "0aibutton"){loadForm('./chatbot/index.html'); }
  if(button.id === "0chat"){loadForm('./chat/chat.html'); }
  if(button.id === "0map"){loadForm('./map/index.html'); }
  if(button.id === "0selflearning"){window.open('https://www.canva.com/', '_self'); }
